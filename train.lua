...
local function train(opt,optimMethod,optimState,data,model,criterion)

	------------------------------------------------------------------------
	-- create model and loss/grad evaluation function
	--
	local n_train_data = data.train_data:size(1) -- number of training data
	local automodel = model

	local x, dl_dx = automodel:getParameters()
	...
	local feval = function(x_new)
	  ...
	  local batch_targets = data.train_targets[{{start_index, end_index}}]
	   ...
	   local loss_x = criterion:forward(automodel:forward(batch_inputs), batch_targets)
	   automodel:backward(batch_inputs, criterion:backward(automodel.output, batch_targets))
	...
	local iterations = epochs * math.ceil(n_train_data / opt.batch_size) -- integer number of minibatches to process
	   ...
	   for i = 1,iterations do
		  ...
		  if i % opt.test_model_iteration == 0 then
		   		local test_outputs = automodel:forward(data.test_data)
    ...
    return automodel, losses, test_losses
end
